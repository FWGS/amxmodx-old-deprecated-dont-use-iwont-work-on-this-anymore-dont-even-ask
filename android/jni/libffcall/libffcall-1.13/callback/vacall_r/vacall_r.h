/* callback/vacall_r/vacall_r.h.  Generated from vacall_r.h.in by configure.  */
...
#if !(defined(__mips64__) || defined(__sparc64__) || defined(__alpha__) || defined(__arm64__) || defined(__powerpc64__) || defined(__ia64__) || (defined(__x86_64__) && !defined(__x86_64_x32__))) && defined(HAVE_LONG_LONG_INT)
    long long           _longlong;
...
#if defined(__i386__) || defined(__m68k__) || (defined(__mips__) && !defined(__mipsn32__) && !defined(__mips64__)) || (defined(__sparc__) && !defined(__sparc64__)) || defined(__alpha__) || defined(__arm__) || defined(__armhf__) || defined(__arm64__) || (defined(__powerpc__) && !defined(__powerpc64__) && (defined(_AIX) || (defined(__MACH__) && defined(__APPLE__)))) || defined(__powerpc64__) || defined(__ia64__) || defined(__x86_64__) || defined(__s390x__)
/* args grow up */
...
#define __va_arg_leftadjusted(LIST,TYPE_SIZE,TYPE_ALIGN)  \
  ((((LIST)->onstack == 0 && (LIST)->aptr >= (long)&(LIST)->iarg[8])	\
...
#define __va_arg_leftadjusted(LIST,TYPE_SIZE,TYPE_ALIGN)  \
  ((((LIST)->onstack == 0 && ((LIST)->aptr + __va_argsize(TYPE_SIZE) > (long)&(LIST)->iarg[5])) \
    ? ((LIST)->onstack=1, (LIST)->aptr = (LIST)->saptr) : 0),		\
   (LIST)->aptr += __va_argsize(TYPE_SIZE),				\
   (LIST)->aptr - __va_argsize(TYPE_SIZE)				\
  )
...
#endif
#if defined(__m68k__) || ((defined(__mipsn32__) || defined(__mips64__)) && defined(_MIPSEB)) || defined(__sparc__) || defined(__sparc64__) || defined(__hppa__) || ((defined(__arm__) || defined(__armhf__)) && !defined(__ARMEL__)) || (defined(__powerpc__) && !defined(__powerpc64__)) || (defined(__powerpc64__) && defined(_BIG_ENDIAN)) || (defined(__s390__) && !defined(__s390x__))
...
#define __va_arg_adjusted(LIST,TYPE_SIZE,TYPE_ALIGN)  \
  (((TYPE_SIZE) <= 2*sizeof(__vaword)					\
    && (LIST)->memiargptr + ((TYPE_SIZE) + sizeof(__vaword)-1) / sizeof(__vaword) <= &(LIST)->iarg[__VA_IARG_NUM]) \
...
#endif
#if defined(__mips__) || defined(__arm__) || defined(__armhf__) || (defined(__powerpc__) && !(defined(_AIX) || (defined(__MACH__) && defined(__APPLE__))))
...
#define va_arg_float(LIST)  \
  ((LIST)->fanum <= 15							\
   ...
   : (((LIST)->aptr < (LIST)->saptr ? (LIST)->aptr = (LIST)->saptr : 0), \
      (LIST)->aptr += sizeof(float),					\
      *(float*)((LIST)->aptr - sizeof(float))				\
  )  )
#define va_arg_double(LIST)  \
...
#endif
#if (defined(__powerpc__) && !defined(__powerpc64__) && (defined(_AIX) || (defined(__MACH__) && defined(__APPLE__)))) || (defined(__powerpc64__) && defined(_AIX))
/* small structures < 1 word are adjusted depending on compiler */
#define __va_arg_struct(LIST,TYPE_SIZE,TYPE_ALIGN)  \
  (__va_align_struct(LIST,TYPE_SIZE,TYPE_ALIGN)				\
...
#define _va_return_struct(LIST,TYPE_SIZE,TYPE_ALIGN,VAL_ADDR)  \
  (__va_return(LIST,__VAstruct),					\
   callback_structcpy((void*)((LIST)->raddr),VAL_ADDR,TYPE_SIZE,TYPE_ALIGN)	\
  )
...
/* On SPARC, PIC compiled code crashes when used outside of a shared library.
   Therefore, don't use the get__vacall_r indirection on this platform. */

extern
#ifdef __cplusplus
"C"
#endif
void __vacall_r (); /* Actually it takes arguments and returns values! */

#define get__vacall_r() (&__vacall_r)

#else

/* A fake type for __vacall_r.
   Actually it takes arguments and returns values. */
typedef void (*__vacall_r_t) (void);

/* This function returns the address of __vacall_r.
   __vacall_r is not a global symbol, because on ELF platforms, functions with
   global visibility cannot accept additional arguments in registers. See
   elf-hack.txt for more details. */
extern
#ifdef __cplusplus
"C"
#endif
__vacall_r_t get__vacall_r (void);

#endif

extern int __va_error1 (enum __VAtype, enum __VAtype);
extern int __va_error2 (unsigned int);
extern void callback_structcpy (void* dest, const void* src, unsigned long size, unsigned long alignment);
